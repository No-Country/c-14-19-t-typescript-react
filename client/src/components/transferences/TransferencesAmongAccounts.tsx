...
const TransferencesAmongAccounts = (): React.ReactElement => {
  ...
  const handleSubmit = async (
    ...
    if (makeTransfer?.status === 201) {
        setIsClicked(false);
        const userCondition = confirm('Transferencia enviada, ¿Desea realizar alguna otra operación?') //! ALERT TEMPORAL
        
        if (userCondition) return window.location.reload();
        else router.push('/customer/homebanking')
    }
  };
